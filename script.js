document.addEventListener("DOMContentLoaded", () => {
  ...
  /* =========================================================
     NEON POT LEAF RAIN — REALISM + WIND GUSTS
  ========================================================= */
  if (neonLeafContainer) {
    ...
    // Wind gust state
    let gusting = false;
    let gustStrength = 0; // pixels added/subtracted to drift
    ...
    function scheduleGust() {
      ...
      setTimeout(() => {
        ...
        // -1 or +1 direction gust
        const dir = Math.random() < 0.5 ? -1 : 1;
        gustStrength = dir * (80 + Math.random() * 180); // 80–260px extra drift

        // Gust lasts 2–3.5 seconds
    ...
    function spawnLeaf() {
      ...
      const size = 10 + depth * 20;          // 10–30px
      const opacity = 0.05 + depth * 0.16;   // 0.05–0.21
      const blur = (1 - depth) * 1.6;        // 0–1.6px
      const duration = 12 + (1 - depth) * 18; // 12–30s

      // drift + wind gust
      const baseDrift = (Math.random() * 260 - 130); // -130..130
      const gustDrift = gusting ? gustStrength : 0;
      const drift = (baseDrift + gustDrift).toFixed(0) + "px";
      ...
      setTimeout(() => leaf.remove(), Math.ceil(duration * 1000) + 4000);
    }

    for (let i = 0; i < 12; i++) setTimeout(spawnLeaf, i * 160);
