document.addEventListener("DOMContentLoaded", () => {
  ...
  // Press & hold logo to reveal members section
  let holdTimer = null;
  let holding = false;
  ...
  function renderCategoryHeader(cat) {
    ...
    header.innerHTML = `
      <div class="menu-section-title">${escapeHtml(String(cat).toUpperCase())}</div>
  ...
  /* =========================================================
     NEON POT LEAF RAIN (NO CLIP VERSION)
  ========================================================= */
  if (neonLeafContainer) {
    ...
    for (let i = 0; i < 8; i++) setTimeout(spawnLeaf, i * 220);
